...
class TaskDriver:
    ...
    def driver_phase3_task4(self):
        ...
        reduction_val = 0
        red_tech = []
        if reduction['use_reduction'] == "yes":
            red_tech = inquirer.prompt(self.reduction_tech)
        ...
        # apply proper reduction technique
        if reduction['use_reduction'] == "yes":
            # use reduction_value instead of k in this part
            if (red_tech['reduction_technique'] == 'SVD'):
                decomposed = self.run_SVD(image_vectors, reduction_val)
                image_vectors = decomposed[0]
                # print("Query Image dimension", q_image_vector.shape)
                # print("dimension of decomposed[0]", decomposed[0].shape, " \ndecomposed[1]", decomposed[1].shape, "\n decomposed[2]", decomposed[2].T.shape)
                # temp = decomposed[]
                q_image_vector = np.matmul(q_image_vector, decomposed[2].T)
                # print("dimension of query ", q_image_vector.shape, "\nquery image in latent space. ", q_image_vector)
                self.save_numpy_array(decomposed[0],
                                      os.path.join(self.out_path,
                                                   "phase3-task2-SVD-" + str(
                                                       feature_model['feature_model']) + "-" + str(k) + '-lev.npy'))
                self.save_numpy_array(decomposed[1],
                                      os.path.join(self.out_path,
                                                   "phase3-task2-SVD-" + str(
                                                       feature_model['feature_model']) + "-" + str(k) + '-ll.npy'))
                self.save_numpy_array(decomposed[2],
                                      os.path.join(self.out_path,
                                                   "phase3-task2-SVD-" + str(
                                                       feature_model['feature_model']) + "-" + str(k) + '-rev.npy'))
            elif (red_tech['reduction_technique'] == 'PCA'):
                decomposed = self.run_PCA(image_vectors, k)
                self.save_numpy_array(decomposed[0],
                                      os.path.join(self.out_path,
                                                   "phase3-task2-PCA-" + str(
                                                       feature_model['feature_model']) + "-" + str(k) + '-lev.npy'))
                self.save_numpy_array(decomposed[1],
                                      os.path.join(self.out_path,
                                                   "phase3-task2-PCA-" + str(
                ...
                self.save_numpy_array(decomposed[2],
                                      os.path.join(self.out_path,
                                                   "phase3-task4-PCA-" + str(
                                                       feature_model['feature_model']) + "-" + str(k) + '-rev.npy'))
            elif (red_tech['reduction_technique'] == 'LDA'):
                decomposed = self.run_LDA(image_vectors, k)
                self.save_numpy_array(decomposed[0],
                                      os.path.join(self.out_path,
                                                   "phase3-task4-LDA-" + str(
                                                       feature_model['feature_model']) + "-" + str(k) + '-lev.npy'))
                self.save_numpy_array(decomposed[1],
                                      os.path.join(self.out_path,
                                                   "phase3-task4-LDA-" + str(
                ...
                decomposed = self.run_k_means(image_vectors, k)
                self.save_k_means_output(images, image_vectors, k, decomposed, "phase3-task4",
                                         str(feature_model['feature_model']), "")
        else:
        ...
        # index = self.find_index_of_query_image(images_folder_path, query_image_path)

        # if index == -1:
        #     print("image not present in folder calculating latent semantics for the same")
        # else:
