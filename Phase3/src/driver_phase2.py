...
class TaskDriver:
    ...
    def driver_task1(self):
        ...
        if ( red_tech['reduction_technique'] == 'SVD' ):
            ...
            self.save_numpy_array(decomposed[0], 
                                  os.path.join(self.out_path , "task1-SVD-"+str(feature_model['feature_model'])+'-'+image_X['image_type']+"-"+str(k)+'-lev.npy'))
            self.save_numpy_array(decomposed[1], 
                                  os.path.join(self.out_path , "task1-SVD-"+str(feature_model['feature_model'])+'-'+image_X['image_type']+"-"+str(k)+'-ll.npy'))
            self.save_numpy_array(decomposed[2], 
                                  os.path.join(self.out_path , "task1-SVD-"+str(feature_model['feature_model'])+'-'+image_X['image_type']+"-"+str(k)+'-rev.npy'))
        elif ( red_tech['reduction_technique'] == 'PCA' ):
            decomposed = self.run_PCA(image_vectors, k)
            self.save_numpy_array(decomposed[0], 
                                  os.path.join(self.out_path , "task1-PCA-"+str(feature_model['feature_model'])+'-'+image_X['image_type']+"-"+str(k)+'-lev.npy'))
            self.save_numpy_array(decomposed[1], 
                                  os.path.join(self.out_path , "task1-PCA-"+str(feature_model['feature_model'])+'-'+image_X['image_type']+"-"+str(k)+'-ll.npy'))
            self.save_numpy_array(decomposed[2], 
                                  os.path.join(self.out_path , "task1-PCA-"+str(feature_model['feature_model'])+'-'+image_X['image_type']+"-"+str(k)+'-rev.npy'))
        elif ( red_tech['reduction_technique'] == 'LDA' ):
            decomposed = self.run_LDA(image_vectors, k)
            print(decomposed[0].shape, decomposed[1].shape)
            self.save_numpy_array(decomposed[0], 
                                  os.path.join(self.out_path , "task1-LDA-"+str(feature_model['feature_model'])+'-'+image_X['image_type']+"-"+str(k)+'-lev.npy'))
            self.save_numpy_array(decomposed[1], 
                                  os.path.join(self.out_path , "task1-LDA-"+str(feature_model['feature_model'])+'-'+image_X['image_type']+"-"+str(k)+'-rev.npy'))
        elif( red_tech['reduction_technique'] == 'k-means'):
            decomposed = self.run_k_means(image_vectors, k)
            self.save_k_means_output(images, image_vectors, k, decomposed, "task1", str(feature_model['feature_model']), image_X['image_type'])
    ...
    def driver_task2(self):
        ...
        if ( red_tech['reduction_technique'] == 'SVD' ):
            decomposed = self.run_SVD(image_vectors, k)
            self.save_numpy_array(decomposed[0], 
                                  os.path.join(self.out_path , "task2-SVD-"+str(feature_model['feature_model'])+'-'+str(image_Y)+"-"+str(k)+'-lev.npy'))
            self.save_numpy_array(decomposed[1], 
                                  os.path.join(self.out_path , "task2-SVD-"+str(feature_model['feature_model'])+'-'+str(image_Y)+"-"+str(k)+'-ll.npy'))
            self.save_numpy_array(decomposed[2], 
                                  os.path.join(self.out_path , "task2-SVD-"+str(feature_model['feature_model'])+'-'+str(image_Y)+"-"+str(k)+'-rev.npy'))
        elif ( red_tech['reduction_technique'] == 'PCA' ):
            decomposed = self.run_PCA(image_vectors, k)
            self.save_numpy_array(decomposed[0], 
                                  os.path.join(self.out_path , "task2-PCA-"+str(feature_model['feature_model'])+'-'+str(image_Y)+"-"+str(k)+'-lev.npy'))
            self.save_numpy_array(decomposed[1], 
                                  os.path.join(self.out_path , "task2-PCA-"+str(feature_model['feature_model'])+'-'+str(image_Y)+"-"+str(k)+'-ll.npy'))
            self.save_numpy_array(decomposed[2], 
                                  os.path.join(self.out_path , "task2-PCA-"+str(feature_model['feature_model'])+'-'+str(image_Y)+"-"+str(k)+'-rev.npy'))
        elif ( red_tech['reduction_technique'] == 'LDA' ):
            decomposed = self.run_LDA(image_vectors, k)
            self.save_numpy_array(decomposed[0], 
                                  os.path.join(self.out_path , "task2-LDA-"+str(feature_model['feature_model'])+'-'+str(image_Y)+"-"+str(k)+'-lev.npy'))
            self.save_numpy_array(decomposed[1], 
                                  os.path.join(self.out_path , "task2-LDA-"+str(feature_model['feature_model'])+'-'+str(image_Y)+"-"+str(k)+'-rev.npy'))
        elif( red_tech['reduction_technique'] == 'k-means'):
            decomposed = self.run_k_means(image_vectors, k)
            self.save_k_means_output(images, image_vectors, k, decomposed, "task2", str(feature_model['feature_model']), str(image_Y))
    ...
    def driver_task3(self):
        ...
        if ( red_tech['reduction_technique'] == 'SVD' ):
            ...
            self.save_numpy_array(image_vectors, 
                                  os.path.join(self.out_path , "task3-SVD-"+str(feature_model['feature_model'])+"-" +str(k)+'-similarity.npy'))
            self.save_numpy_array(type_image_vectors.T, 
                                  os.path.join(self.out_path , "task3-SVD-"+str(feature_model['feature_model'])+"-" +str(k)+'-transform-1.npy'))
            self.save_numpy_array(decomposed[0], 
                                  os.path.join(self.out_path , "task3-SVD-"+str(feature_model['feature_model'])+"-" +str(k)+'-lev.npy'))
            self.save_numpy_array(decomposed[1], 
                                  os.path.join(self.out_path , "task3-SVD-"+str(feature_model['feature_model']) + "-" +str(k)+'-ll.npy'))
            self.save_numpy_array(decomposed[2], 
                                  os.path.join(self.out_path , "task3-SVD-"+str(feature_model['feature_model']) + "-" + str(k)+'-rev.npy'))
        elif ( red_tech['reduction_technique'] == 'PCA' ):
            decomposed = self.run_PCA(image_vectors, k)
            self.save_numpy_array(image_vectors, 
                                  os.path.join(self.out_path , "task3-PCA-"+str(feature_model['feature_model'])+"-" +str(k)+'-similarity.npy'))
            self.save_numpy_array(type_image_vectors.T, 
                                  os.path.join(self.out_path , "task3-PCA-"+str(feature_model['feature_model'])+"-" +str(k)+'-transform-1.npy'))
            self.save_numpy_array(decomposed[0], 
                                  os.path.join(self.out_path , "task3-PCA-"+str(feature_model['feature_model'])+"-"+str(k)+'-lev.npy'))
            self.save_numpy_array(decomposed[1], 
                                  os.path.join(self.out_path , "task3-PCA-"+str(feature_model['feature_model']) +"-"+str(k)+'-ll.npy'))
            self.save_numpy_array(decomposed[2], 
                                  os.path.join(self.out_path , "task3-PCA-"+str(feature_model['feature_model'])+ "-" + str(k)+'-rev.npy'))
        elif ( red_tech['reduction_technique'] == 'LDA' ):
            decomposed = self.run_LDA(image_vectors, k)
            self.save_numpy_array(image_vectors, 
                                  os.path.join(self.out_path , "task3-LDA-"+str(feature_model['feature_model'])+"-" +str(k)+'-similarity.npy'))
            self.save_numpy_array(type_image_vectors.T, 
                                  os.path.join(self.out_path , "task3-LDA-"+str(feature_model['feature_model'])+"-" +str(k)+'-transform-1.npy'))
            self.save_numpy_array(decomposed[0], 
                                  os.path.join(self.out_path , "task3-LDA-"+str(feature_model['feature_model'])+ "-" +str(k)+'-lev.npy'))
            self.save_numpy_array(decomposed[1], 
                                  os.path.join(self.out_path , "task3-LDA-"+str(feature_model['feature_model'])+"-"+str(k)+'-rev.npy'))
        elif( red_tech['reduction_technique'] == 'k-means'):
            decomposed = self.run_k_means(image_vectors, k)
            self.save_numpy_array(image_vectors, 
                                  os.path.join(self.out_path , "task3-k-means-"+str(feature_model['feature_model'])+'-type-'+str(k)+'-similarity.npy'))
            self.save_numpy_array(type_image_vectors.T, 
                                  os.path.join(self.out_path , "task3-k-means-"+str(feature_model['feature_model'])+'-type-'+str(k)+'-transform-1.npy'))
            self.save_k_means_output(images, image_vectors, k, decomposed, "task3", str(feature_model['feature_model']), "type")
    ...
    def driver_task5(self):
        ...
        latent_sem_file = input("Input the latent semantics filename in the form of (task)-(reduction technique)-(feature model)-(type/ID)-(k)e.g. task1-PCA-HOG-emboss-5: ")
